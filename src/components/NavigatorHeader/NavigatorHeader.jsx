...
function NavigatorHeader() {
    ...
    return(
        <div className='fixed top-0 left-0 right-0 flex w-full h-[50px] justify-center bg-black md:hidden '>
            <ArrowLeft className='text-white items-center absolute top-3 left-1' />
            <Link to = "/" className = " w-[50px] h-[50px] "><img src={logo} alt="LogoThreads" /></Link>
            <button className=' text-white items-center absolute top-3 right-1 ' onClick={handleopenMenu} >
                <Menu className=' text-white' />
            </button>
            {isopenMenu && (<>
                 <div 
                        className='fixed inset-0 top-[50px]' 
                        onClick={handlecloseMenu}
                    />
                 <ul className='absolute text-white top-9 right-0 bg-stone-700 rounded-lg shadow-lg p-4 w-48 flex flex-col justify-end'>
